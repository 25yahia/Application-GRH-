...
public class FileStorageServiceImpl implements FileStorageService {
	...
	public Stream<Path> loadAll() {
	 ...
	 @Override
	    public void save(MultipartFile file, Long candidatId) {
	        try {
	            Candidat candidat = candidatRepository.findById(candidatId)
	                                                  .orElseThrow(() -> new RuntimeException("Candidat not found!"));
	            
	            FileInfo fileInfo = new FileInfo(file.getOriginalFilename(), "uploads/" + file.getOriginalFilename());
	            fileInfo.setCandidat(candidat);

	            fileinfoRepository.save(fileInfo);
	        } catch (Exception e) {
	            throw new RuntimeException("Could not save the file: " + file.getOriginalFilename(), e);
	        }
	    }
